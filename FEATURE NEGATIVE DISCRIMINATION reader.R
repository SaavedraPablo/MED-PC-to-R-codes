...
#This part selects the file containing the MED-PC output file from a single session in the feature negative discrimination task and names it
...
colnames(type_matrix) <- c("Trial_type")

#Merging both matrixes and creating a dataframe
FND_matrix<-cbind(type_matrix,matrix_no_number)
FND_Dataframe<-data.frame(FND_matrix)